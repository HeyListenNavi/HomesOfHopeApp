...
export const VisitFamilyCard = ({ family }: VisitFamilyCardProps) => {
  const isVisited = family.status === "visited" || family.status === "closed";
  ...
  return (
    <TouchableOpacity
      className="p-4 rounded-2xl gap-4 bg-white mb-4 border border-gray-100 shadow-sm"
      // 1. CORRECCIÓN: Navegación dinámica usando el ID real
      onPress={() => router.push(`/family-profile/${family.id}`)}
    >
      <View className="flex-row items-center gap-3">
        {/* Indicador de Índice / Estatus */}
        <View
          className={`w-10 h-10 rounded-full items-center justify-center ${
            isVisited ? "bg-primary" : "bg-gray-100"
          }`}
        >
        </View>

        {/* 2. CORRECCIÓN: Uso de datos mapeados (name y location) */}
        <View className="flex-1">
          ...
          <View className="flex-row items-center gap-1">
             <Boxicon name="bxs-map" size={12} color="#9ca3af" />
      ...
      <View className="flex-row justify-between gap-2">
        <TouchableOpacity className="flex-row items-center flex-1 gap-1 px-3 py-3 bg-[#9BD189]/10 rounded-xl justify-center">
          <BrandBoxicon
            name="bx-whatsapp"
            size={22}
            color="#61b346"
          />
          <Text className="text-primary font-bold text-xs">WhatsApp</Text>
        </TouchableOpacity>

        <TouchableOpacity className="flex-row items-center flex-1 gap-1 px-3 py-3 bg-blue-50 rounded-xl justify-center">
          <Boxicon name="bxs-location" size={22} color="#3b82f6" />
          <Text className="text-[#3b82f6] font-bold text-xs">Mapa</Text>
        </TouchableOpacity>

        {/* Dialog de Check-in / Confirmar Visita */}
        <Dialog>
              ...
              >
                <DialogHeader className="items-center gap-3">
                  ...
                  <DialogDescription className="text-gray-500 text-center text-base">
                    ¿Deseas marcar a <Text className="font-bold text-gray-700">{family.family_name}</Text> como visitada?
                  </DialogDescription>
                </DialogHeader>
                ...
                <View className="py-4 gap-3">
                  <View>
                    <Textarea
                      ...
                      children={
                        <Text className="ml-auto text-sm text-gray-300">
                          ...
                          {voiceRecorder.recordedUri ? "1 audio" : "Sin audio"}
                        </Text>
                      }
                    ...
                    <View className="py-2 flex-row gap-2">
                      ...
                      >
                        <Boxicon name="bxs-camera" size={20} color="#4b5563" />
                      ...
                      {!voiceRecorder.recordedUri ? (
                        ...
                        >
                          <Boxicon
                            ...
                            size={24}
                            color={
                              voiceRecorder.isRecording ? "#ef4444" : "#4b5563"
                            }
                          />
                          {voiceRecorder.isRecording ? (
                            <View className="flex-row items-center gap-1">
                    ...
                    <FlatList
                      ...
                      renderItem={({ item: uri, index }) => (
                        ...
                        >
                          ...
                          >
                            <Boxicon name="bx-x" size={16} color="#ef4444" />
                          </TouchableOpacity>
                        </View>
                      )}
                    />
